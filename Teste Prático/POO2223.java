...
public class POO2223 {
	...
	private static void alinea2(PrintStream out) {
		...
		*/
		try {
            Scanner sc = new Scanner(new FileReader("bookings.txt"));
            ...
            while(sc.hasNextLine()){
				...
				int numero_contribuinte = Integer.parseInt(parts[0]);
				LocalDate data_inicio = LocalDate.parse(parts[1]);
				int duracao = Integer.parseInt(parts[2]);
				String tipo = parts[3];
				String dimensoes = parts[4];
				String[] dim = dimensoes.split("x");
				int[] dimensoes_int = new int[2];
				dimensoes_int[0] = Integer.parseInt(dim[0]);
				dimensoes_int[1] = Integer.parseInt(dim[1]);
				Client cliente = campingService.getClient(numero_contribuinte);

				if (cliente == null) {
					out.printf("** Client with tax number %d does not exist.\n", numero_contribuinte);
					continue;
				}
				else {
					CampingSpace espaco = campingService.findAvailableCampingSpaces(SpaceType.valueOf(tipo), data_inicio, duracao, dimensoes_int).get(0);

					campingService.bookCampingSpace(cliente, espaco, data_inicio, duracao);
				}
			}
